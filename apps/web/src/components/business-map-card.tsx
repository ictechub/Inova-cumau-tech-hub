...
const LAND_HOVER   = "var(--map-land-hover)"
const LAND_ACTIVE  = "var(--map-land-active)"
const MAP_BORDER   = "var(--map-border)"
const MAP_LABEL    = "var(--map-label)"
      ...
      return (
        ...
        >
          ...
          <circle cx={x} cy={y} r={isHover ? DOT_R * 1.15 : DOT_R}
            style={{ fill: PRIMARY, filter: `drop-shadow(0 0 ${DOT_R * 0.4}px color-mix(in srgb, var(--color-primary-foreground) 40%, transparent))` }}
            fillOpacity={1} />
        </g>
      )
    })
  ...
  return (
    <Card ref={cardRef} className={cn("map-fullscreen-card", isFullscreen && "rounded-none flex flex-col")}>
      <CardHeader className="py-4 border-b shrink-0">
        <CardTitle className="font-sans! text-base">Negócios por região</CardTitle>
        <p className="text-sm text-muted-foreground mt-0.5">
          Distribuição geográfica das startups associadas
        </p>
      </CardHeader>
      ...
      <CardContent className={cn("px-6 pt-5 pb-6", isFullscreen && "flex-1 flex flex-col overflow-hidden")}>
        ...
        <div className={cn("flex border rounded-lg overflow-hidden", isFullscreen ? "flex-1 min-h-0" : "h-[400px]")}>
          ...
          {/* Mapa SVG */}
          <div ref={mapWrapRef} className="relative flex-1 overflow-hidden" style={{ background: "var(--map-bg)" }}>
            {/* Fullscreen */}
            <Button
              variant="outline"
              size="icon"
              ...
              {metaReady && SHOW_STATE && BR_STATES.map(st => {
                ...
                return (
                  <text key={`sl-${st.id}`} x={m.cx} y={m.cy} textAnchor="middle"
                    ...
                    style={{ fill: MAP_LABEL, paintOrder: "stroke", stroke: "var(--map-bg)", strokeWidth: fontWorld(2) }}>
                    {text}
                  </text>
                )
              })}
              ...
              {SHOW_CITY && visibleDots.map(city => {
                ...
                return (
                  <text key={`city-${city.state}-${city.city}`}
                    ...
                    style={{ fill: "var(--color-foreground)", paintOrder: "stroke", stroke: "var(--map-bg)", strokeWidth: fontWorld(2.5) }}>
                    {city.city}
                  </text>
                )
              })}
                ...
                <div className="flex-1 overflow-y-auto map-scroll px-5 pb-5 space-y-3">
                  {topRegions.map(r => (
                    <div key={r.id} className="space-y-1 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => selectRegion(r.id)}>
                      ...
                      <p className="text-[10px] text-muted-foreground">{r.count} {r.count === 1 ? "negócio" : "negócios"}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {level === "region" && (
              <div className="flex flex-col h-full overflow-hidden">
                <div className="px-5 pt-4 pb-3 border-b shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{regionTotal === 1 ? "negócio" : "negócios"} na região</p>
                </div>
                <div className="flex-1 overflow-y-auto map-scroll px-5 py-4 space-y-3">
                  {statesOfRegion.length === 0
                    ? <p className="text-xs text-muted-foreground text-center pt-4">Nenhum negócio nessa região.</p>
            ...
            {level === "state" && (
              <div className="flex flex-col h-full overflow-hidden">
                <div className="px-5 pt-4 pb-3 border-b shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{stateTotal === 1 ? "negócio" : "negócios"} no estado</p>
                </div>
                <div className="flex-1 overflow-y-auto map-scroll px-5 py-4 space-y-3">
            ...
            {level === "city" && selectedCity && (
              <div className="flex flex-col h-full overflow-hidden">
                <div className="px-5 pt-4 pb-3 border-b shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{selectedCity.count === 1 ? "negócio" : "negócios"} na cidade</p>
                </div>
                <div className="flex-1 overflow-y-auto map-scroll px-5 py-4">
                  {stateTotal > 0 && (
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      {Math.round((selectedCity.count / stateTotal) * 100)}% dos negócios de {selectedCity.state}.
                    </p>
                  )}
                </div>
              </div>
