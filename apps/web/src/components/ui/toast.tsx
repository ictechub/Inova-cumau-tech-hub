...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed top-4 right-4 z-50 w-[calc(100%-2rem)] max-w-sm outline-none",
        className
      )}
      {...props}
    />
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") {
    icon = (
      <IconCircleCheck className="text-success-600" aria-hidden="true" />
    )
  }

  if (type === "info") {
    icon = (
      <IconInfoCircle className="text-info-600" aria-hidden="true" />
    )
  }

  if (type === "warning") {
    icon = (
      <IconAlertTriangle className="text-warning-600" aria-hidden="true" />
    )
  }

  if (type === "error") {
    icon = (
      <IconCircleX className="text-error-600" aria-hidden="true" />
    )
  }
