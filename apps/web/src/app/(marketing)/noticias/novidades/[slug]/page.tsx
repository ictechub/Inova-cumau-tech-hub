...
async function getNovidade(slug: string): Promise<NovidadeDetalhe | null> {
...
export async function generateMetadata({
  ...
  const novidade = await getNovidade(slug);

  if (!novidade) return { title: "Novidade | Inova Cumaú" };

  return { title: `${novidade.title} | Inova Cumaú` };
}
...
export default async function NovidadePage({
  ...
  const autorNome = novidade.show_author ? await getAutorNome(novidade.owner_id) : null;

  const html = generateHTML(
    (novidade.content as Record<string, unknown>) ?? { type: "doc", content: [] },
    tiptapExtensions,
  );
  ...
  return (
    <article className="mx-auto max-w-2xl px-4 pt-16 pb-20 sm:px-6">
      ...
      <h1 className="mt-3 font-serif text-3xl font-medium sm:text-4xl">{novidade.title}</h1>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {novidade.published_at ? (
          <span className="text-sm text-muted-foreground">{formatData(novidade.published_at)}</span>
        ) : null}
      </div>
